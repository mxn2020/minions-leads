---
name: minions-leads
description: Agent skills for working with Minions Leads MinionTypes. Provides CRUD operations, CLI usage, and best practices for AI agents managing minions-leads data.
---

# Minions Leads Agent Skills

Skills for agents operating on the `minions-leads` toolbox.
...
The `leads` CLI provides basic project info and utilities:
...
```ts
import { customTypes } from '@minions-leads/sdk/schemas';
...
```python
from minions_leads.schemas import custom_types
...
- This agent only writes to `minions-leads` — it reads from other toolboxes but never writes to them
